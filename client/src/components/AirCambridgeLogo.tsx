...
export default function AirCambridgeLogo({ className, showTagline = false }: AirCambridgeLogoProps) {
  return (
    <div className={cn("flex items-center md:flex-col md:items-center gap-0 md:gap-2 flex-shrink-0", className)}>
      <img 
        src={brandLogo} 
        alt="AirCambridge" 
        className="h-7 sm:h-8 md:h-12 w-auto object-contain max-w-[120px] sm:max-w-[140px] md:max-w-none" 
      />
      {showTagline && (
